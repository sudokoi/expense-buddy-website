              ...
              return (
                <g key={series.id} opacity={isSelected ? 1 : 0.24}>
                  ...
                  {series.points.map((point) => (
                    ...
                    >
                      <title>
                        {series.label} · {point.label}: {formatCurrencyValue(point.value, currency)}
                      </title>
                    </circle>
                  ))}
                </g>
        ...
        <div className="space-y-3">
          ...
          {data.series.map((series) => {
            ...
            return (
              ...
              >
                <span className="flex min-w-0 items-center gap-3 text-sm text-white/78">
                  <span
            ...
            {items.map((item, index) => {
              ...
              return (
                <g
                  ...
                  onPointerEnter={() => setActiveCategory(item.category)}
                ...
                >
                  ...
                  <rect width={xScale(item.value)} height={18} rx={9} fill={color} opacity={0.95} />
                  <text
      ...
      <CardContent className="space-y-5">
        ...
        >
          ...
          <g transform={`translate(${centerX}, ${centerY})`} filter={`url(#${chartId}-shadow)`}>
            ...
            <circle r={48} fill="#171423" />
            <text y={-4} textAnchor="middle" className="analytics-axis-text">
              Leading share
            </text>
            <text y={22} textAnchor="middle" className="fill-white text-[24px] font-semibold">
              {activeItem ? formatPercentage(activeItem.percentage) : '0%'}
            </text>
          </g>
        </svg>
            ...
            {points.map((point, index) => {
              ...
              return (
                <g key={point.date} transform={`translate(${x}, 0)`}>
                  ...
                  >
                    <title>
                      {point.label}: {formatCurrencyValue(point.value, currency)}
                    </title>
                  </rect>
                </g>
              )
